...
export default function Navbar() {
  ...
  return (
    <motion.nav 
      ...
      className="sticky top-0 z-50 w-full border-b border-slate-800 bg-[#030712]/80 backdrop-blur-xl transition-colors duration-300 text-slate-100"
    >
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex items-center gap-3 md:gap-4 group">
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] mt-1 text-slate-400">
                Digital Success
              </span>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center space-x-6">
            ...
            <div className="relative group/dropdown py-2">
              ...
              <div className="absolute top-full left-0 w-64 pt-2 z-50 opacity-0 pointer-events-none scale-95 origin-top-left group-hover/dropdown:opacity-100 group-hover/dropdown:pointer-events-auto group-hover/dropdown:scale-100 transition-all duration-200">
                <div className="bg-[#111827] border border-slate-800 rounded-2xl shadow-xl p-2">
                  <Link href="/programs#m365" className="flex items-center gap-2.5 px-4 py-3 text-sm font-bold rounded-xl hover:bg-slate-800 transition-colors">
                    <FileCode className="w-4 h-4 text-blue-500" />
                    MS 365 Certification
                  </Link>
                  
                  {/* Updated Target Link */}
                  <Link href="/programs#m365#cambridge" className="flex items-center gap-2.5 px-4 py-3 text-sm font-bold rounded-xl hover:bg-slate-800 transition-colors">
            ...
            <div className="flex items-center gap-3 border-l border-slate-800 pl-6">
              <a href="tel:+94710525968" className="w-10 h-10 rounded-full flex items-center justify-center bg-slate-800 hover:bg-green-500 hover:text-white transition-all" aria-label="Call Key Institute">
                <AnimatedPhone 
                  className="w-4 h-4" 
          ...
          <div className="flex items-center lg:hidden gap-3">
            <a href="tel:+94710525968" className="w-10 h-10 rounded-xl flex items-center justify-center bg-slate-800 text-slate-200 hover:bg-green-500 hover:text-white transition-all" aria-label="Call Key Institute">
              <AnimatedPhone 
            ...
            <button onClick={() => setIsOpen(!isOpen)} className="p-2.5 rounded-xl bg-slate-800 text-slate-200">
        ...
        {isOpen && (
          ...
          >
            <div className="px-4 pb-6 py-4 space-y-1">
              ...
              <Link href="/programs" onClick={() => setIsOpen(false)} className="flex items-center gap-2.5 p-3 font-bold text-sm hover:bg-slate-800 rounded-xl transition-colors">
                <BookOpen className="w-4 h-4 opacity-70" /> Programs
              </Link>
              
              <Link href="/programs#m365" onClick={() => setIsOpen(false)} className="flex items-center gap-2.5 pl-9 pr-3 py-2 font-bold text-sm text-slate-400 hover:text-brand-blue">
                <FileCode className="w-4 h-4 text-blue-500 shrink-0" /> MS 365 Certification
              </Link>
              
              {/* Updated Target Link (Mobile) */}
              <Link href="/programs#m365#cambridge" onClick={() => setIsOpen(false)} className="flex items-center gap-2.5 pl-9 pr-3 py-2 font-bold text-sm text-slate-400 hover:text-brand-blue">
              ...
              {navLinks.slice(1).map((link) => {
                const LinkIcon = link.icon;
                return (
                  <Link key={link.name} href={link.href} onClick={() => setIsOpen(false)} className="flex items-center gap-2.5 p-3 font-bold text-sm hover:bg-slate-800 rounded-xl transition-colors">
                    <LinkIcon className="w-4 h-4 opacity-70" />
                    {link.name}
                  </Link>
                );
              ...
              <div className="grid grid-cols-5 gap-2 mt-4">
                <a href="tel:+94710525968" className="col-span-2 flex items-center justify-center gap-2 p-4 font-bold text-sm rounded-xl border border-slate-800 text-slate-200 hover:bg-green-600 transition-colors">
                  <AnimatedPhone 
                    className="w-4 h-4" 
                    animate={{ rotate: [0, -12, 10, -10, 8, -4, 4, 0] }}
                    transition={{
